...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def accumulate(acc: Int, article: WikipediaArticle): Int = {
        if (article.mentionsLanguage(lang)) acc + 1 else acc
    }
    def combine(n: Int, m: Int): Int = n + m
    rdd.aggregate(0)(accumulate, combine)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(article =>
      langs.filter(article.mentionsLanguage(_)).map((_, article))
    ).groupByKey
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.mapValues( _.size ).collect().sortWith((x,y) => x._2 > y._2).toList
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap( article =>
      langs.map( lang =>
        (lang, article.mentionsLanguage(lang)))
    ).filter(_._2)
    .mapValues(Function.const(1))
    .reduceByKey(_ + _)
    .collect.sortWith((x,y) => x._2 > y._2).toList
  }
